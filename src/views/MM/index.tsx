    ...
    return (
        <div className='post-patch-box'>
            <div className='post-patch-input-container'>
                <div className='input-box'>
                    <div className='input-label'>용품명</div>
                    <input className='input' placeholder='용품명을 입력해주세요' value={name} onChange={onNameChangeHandler}/>
                </div>
                <div className='input-box' style={{ flex: 1 }}>
                    <div className='input-label'>용도</div>
                    <input className='input' placeholder='용도를 입력해주세요' value={purpose} onChange={onPurposeChangeHandler}/>
                </div>
                <div className='input-box'>
                    <div className='input-label'>개수</div>
                    <input className='input' placeholder='개수를 입력해주세요' value={count} onChange={onCountChangeHandler}/>
                </div>
            </div>
...
interface TableRowProps {
    tool: Tool;
    getToolList: () => void;
    onUpdateButtonClickHandler : (toolNumebr:number ) => void
}

// component: 용품 리스트 아이템 컴포넌트 //
function TableRow({ tool, onUpdateButtonClickHandler,getToolList }: TableRowProps) {
    //state : cookie 상태 //
    const [cookies] = useCookies();
...
export default function MM() {
    ...
    const unshowPathcBox = () => setShowPatchBox(false);
    ...
    //event handler : 검색 엔터 클릭 버튼 역할 동일 처리 함수 //
    ...
    return (
        <div id='mm-wrapper'>
            ...
            {showPatchBox && <PatchBox unShow={unshowPathcBox} toolNumber={patchToolNumber} getToolList={getToolList}/>}
            <div className='top'>
            ...
            <div className='bottom'>
                ...
                <div className='search-box'>
                    <input className='search-input' value={searchWord} placeholder='검색어를 입력하세요.' onChange={onSearchWordChangeHandler} onKeyDown={onSearchWordKeyDownHandler}/>
